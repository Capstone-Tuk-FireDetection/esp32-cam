...
// LED FLASH 관련 설정 (LED 사용이 활성화 되었을 때)
#if CONFIG_LED_ILLUMINATOR_ENABLED
#define LED_LEDC_GPIO            22      // LED 연결된 GPIO 핀
#define CONFIG_LED_MAX_INTENSITY 255     // LED 최대 밝기 값

int led_duty = 0;      // 현재 LED 밝기 duty 값
bool isStreaming = false;  // 스트리밍 중인지 여부
#endif
...
void enable_led(bool en) {
  int duty = en ? led_duty : 0;   // LED 켜면 현재 설정값, 끄면 0
  // 스트리밍 중이면 최대 밝기 제한 적용
  if (en && isStreaming && (led_duty > CONFIG_LED_MAX_INTENSITY)) {
    duty = CONFIG_LED_MAX_INTENSITY;
  }
  // LED PWM duty 값을 설정하여 밝기 제어
  ledcWrite(LED_LEDC_GPIO, duty);
  // (다른 LEDC 설정 함수들도 주석 처리되어 있음)
...
#if CONFIG_LED_ILLUMINATOR_ENABLED
  // LED를 켜서 밝기를 높여 캡처 효과 개선
  enable_led(true);
  // LED가 켜진 후 충분히 밝아질 때까지 대기 (~150ms)
  vTaskDelay(150 / portTICK_PERIOD_MS);
  fb = esp_camera_fb_get();  // 캡처된 프레임 가져오기
  enable_led(false);         // 캡처 후 LED 끄기
#else
  fb = esp_camera_fb_get();  // LED 기능 없으면 바로 프레임 캡처
#endif
...
#if CONFIG_LED_ILLUMINATOR_ENABLED
  isStreaming = true;
  enable_led(true);  // 스트리밍 시작 전 LED 켜기
#endif
...
#if CONFIG_LED_ILLUMINATOR_ENABLED
  isStreaming = false;
  enable_led(false);  // 스트리밍 종료 시 LED 끄기
#endif
...
// LED 플래시를 초기화하는 함수 (핀이 연결된 LEDC 채널에 LED를 설정)
void setupLedFlash(int pin) {
#if CONFIG_LED_ILLUMINATOR_ENABLED
  // 지정된 핀에 LEDC 기능을 할당 (5000Hz, 8비트 해상도)
